...
module GitLighthouse
  module Lighthouse
    ...
    class Ticket < Base
      ...
      # because they changed the api for some reason
      def body
        #puts self.inspect
        self.versions.first.body rescue ''
      end
      ...
      def attachments(html_url)
        atts = []
        doc = Hpricot(open(html_url))
        (doc/"ul.attachments"/:li/:ins/:h4/:a).each do |t| 
          arr = t['href'].split('/')
          name = arr.pop
          number = arr.pop
          atts << Attachment.new(name, number)
        end
        atts
      end
  ...
  class Connection
    private
      def authorization_header
        (GitLighthouse::Lighthouse.email || GitLighthouse::Lighthouse.password ? { 'Authorization' => 'Basic ' + ["#{GitLighthouse::Lighthouse.email}:#{GitLighthouse::Lighthouse.password}"].pack('m').delete("\r\n") } : {})
      end
  end
end